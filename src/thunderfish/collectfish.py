...
def collect_fish(files, simplify_file=False,
    ...
    for zf, file_name, fish_type in file_iter(files):
        ...
        # data:
        if zf is not None:
            file_name = io.TextIOWrapper(zf.open(os.fspath(file_name), 'r'))
        ...
        if not table:
            ...
            if fish_type == 'wave':
                if harmonics is not None:
                    ...
                    if zf is not None:
                        fn = io.TextIOWrapper(zf.open(os.fspath(fn), 'r'))
                ...
                if phases0 is not None:
                    fn = base_path.with_name(base_path.stem + '-pulsephases-0' + file_ext)
                    if zf is not None:
                        fn = io.TextIOWrapper(zf.open(os.fspath(fn), 'r'))
                ...
                if phases0 is not None:
                    ...
                    for p in range(phases0, phases1 + 1):
                        for pr in range(pulse_phases.rows()):
                            if pulse_phases[pr, 'index'] == p:
                                break
                        else:
                            continue
                        if p != 1:
                            table.add(pulse_phases[pr,'time'], f'P{p}time')
                        table.add(pulse_phases[pr,'amplitude'], f'P{p}ampl')
                        if p != 1:
                            table.add(pulse_phases[pr,'relampl'], f'P{p}relampl')
                        table.add(pulse_phases[pr,'width'], f'P{p}width')
                if len(pulse_names) > 0:
                    fn = base_path.with_name(base_path.stem + f'-eodwaveform-{idx}' + file_ext)
                    if zf is not None:
                        fn = io.TextIOWrapper(zf.open(os.fspath(fn), 'r'))
