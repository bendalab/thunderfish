...
def plot_clustering(rate, eod_widths, eod_hights, eod_shapes, disc_masks, merge_masks):
    ...
    for i, (wl, colw, uhl, eod_h, eod_h_labs, w_snip, w_feat, w_lab, w_dm, w_mm) in enumerate(zip(eod_widths[0], colidxsw, eod_hights[0], eod_hights[1], eod_hights[2], eod_shapes[0], eod_shapes[1], eod_shapes[2], disc_masks, merge_masks)):
        ...
        # determine total size of the hight histograms now.
...
def plot_bgm(x, means, variances, weights, use_log, labels, labels_am, xlab):
    ...
    # plot the gaussians
    for i, c in enumerate(np.unique(classes)):
        ax2.plot(xplot, gaussians[c], c=lighter(c_grey, colidxs[i]),
                 linewidth=2, label=r'$N(\mu_%i, \sigma_%i)$'%(c, c))
        
    # plot intersection lines
    ax2.vlines(xplot[1:][np.diff(classes)!=0], 0, gmax/gmin,
    ...
    for i, l in enumerate(np.unique(labels)):
        if use_log:
            ...
            h, binn, _ = ax1.hist(x[labels==l],
                                  bins=np.linspace(np.min(x), np.max(x), 100),
                                  color=lighter(ccol, colidxs[i]),
                                  label=r'$x_%i$'%l)
