...
def plot_data(wave_psd_prop, pulse_psd_prop, multi_wave_psd_prop, wave_p2v_prop, pulse_p2v_prop,
    ...
    ax.plot(wave_psd_prop, wave_p2v_prop, 'o', color='royalblue', label='Wave-type', alpha=0.7, markersize=20, mec='k',
            mew=3)
    ax.plot(pulse_psd_prop, pulse_p2v_prop, 'o', color='firebrick', label='Pulse-type', alpha=0.7, markersize=20,
            mec='k', mew=3)
    ax.plot(mixed_psd_prop, mixed_p2v_prop, 'o', color='darkorange', label='Mixed', alpha=0.7, markersize=17, mec='k',
            mew=3)
    ...
    ax.set_title('Relation between\n' + r'$\langle \rho/P \rangle$' + ' and ' + r'$\langle \tau/T \rangle$',
                 fontsize=fs + 2)
    ax.set_ylim([0, 0.5])
...
def plot_psd_algo(wave_prop, pulse_prop, multi_wave_prop, mixed_prop):
    ...
    sns.violinplot(data=dafr, ax=ax1, inner=None, linewidth=2, color='k', palette=("royalblue", "firebrick"),
                   scale="width")
    ...
    ax2.axis([0, 1500, min(pulse_psd_data[1][:len(pulse_psd_data[0][pulse_psd_data[0] < 1500])]),
              max(pulse_psd_data[1]) + 10])
    ...
    ax3.axis(
        [0, 1500, min(wave_psd_data[1][:len(wave_psd_data[0][wave_psd_data[0] < 1500])]), max(wave_psd_data[1]) + 10])
...
def plot_p2v_algo(wave_p2v_prop, pulse_p2v_prop, multi_wave_p2v_prop):
    ...
    sns.violinplot(data=dafr2, ax=ax1, inner=None, linewidth=2, color='k', palette=("royalblue", "firebrick"),
                   scale="width")
    ...
    ax3.plot([peaks[0][0], troughs[1][0]], [-troughs[1][1], -troughs[1][1]], '-', color='royalblue', label=r'$\tau$',
             lw=5)
    plt.legend(bbox_to_anchor=(1, 1.2), ncol=2)

    ax3.plot(np.arange(500) * 1.0 / samplerate * 1000, wave_trace_data[:500] / 2.0 ** 15, lw=2, color='royalblue',
             alpha=0.9)
    ax3.set_xlim([0, 11])
