...
def plot_data(wave_psd_prop, pulse_psd_prop, multi_wave_psd_prop, wave_p2v_prop, pulse_p2v_prop,
    ...
    ax.plot(multi_wave_psd_prop, multi_wave_p2v_prop, 'o', color = 'green', label='multi wave-type', alpha= 0.7, markersize= 5)
    ax.plot(pulse_psd_prop, pulse_p2v_prop, 'o', color= 'red', label= 'pulse-type', alpha= 0.7, markersize= 5)
    # ax.plot(mixed_psd_prop, mixed_p2v_prop, 'o', color = 'k', label='wave-type', alpha= 1, markersize= 6)
    ...
    # ax2.plot(multi_wave_psd_prop, multi_wave_hist_algor, 'o', color = 'green', label='multi wave-type', alpha= 0.7, markersize=5)
    # ax2.plot(mixed_psd_prop, mixed_hist_algor, 'o', color = 'k', label='wave-type', alpha= 1, markersize=6)
    ...
    # ax3.plot(multi_wave_p2v_prop, multi_wave_hist_algor, 'o', color = 'green', label='multi wave-type', alpha= 0.7, markersize=5)
    # ax3.plot(mixed_p2v_prop, mixed_hist_algor, 'o', color = 'k', label='wave-type', alpha= 1, markersize=6)
...
def plot_psd_algo(wave_prop, pulse_prop, multi_wave_prop):
    inch_factor = 2.54
    sns.set_context("poster")
    sns.axes_style('white')
    # sns.set_style("ticks")
    ...
    dafr = pd.DataFrame([wave_prop, multi_wave_prop, pulse_prop]) #turn
    dafr = dafr.transpose()
...
def plot_p2v_algo(wave_p2v_prop, pulse_p2v_prop, multi_wave_p2v_prop):
    ...
    dafr2 = pd.DataFrame([wave_p2v_prop, multi_wave_p2v_prop, pulse_p2v_prop])
    dafr2 =dafr2.transpose()
    dafr2.columns = ['wave', 'multi wave', 'pulse']
    sns.violinplot(data=dafr2, ax=ax1, col=("blue", "green", "red"))
    ...
    ax2.plot(np.arange(3000) * 1.0 / samplerate * 1000, pulse_trace_data[:3000] / 2.0 ** 15, lw=2, color='red', alpha=0.7)
    ax2.set_xlabel('time [ms]')
    ax2.set_ylabel('Amplitude [a.u.]')
...
def main():
    ...
    plot_psd_algo(wave_psd_prop, pulse_psd_prop, multi_wave_psd_prop)
