...
def create_movie(ident_v, idents, idx_v, times, x_ident, y_ident, fund_v, test = False):
    l0 = None
    l1 = None
    l2 = None
    ...
    meta = []
    for ident in idents:
        meta.append([ident, None, [None, None, None, None, None, None], None, None, None]) # [identity, color, marker_handles, trace_handle, text_handle, path]
    ...
    with writer.saving(fig, "/home/raab/Desktop/" + mov_name, 300):
        ...
        for i in tqdm(range(frames), desc='create images'):
            for j in range(len(idents)):
                if not meta[j][2][0] == None:
                    meta[j][2][0].remove()
                    meta[j][2][0] == None
                ...
                if ~np.isnan(x_ident[j][i]):
                    if len(fund_v[(ident_v == idents[j]) & (idx_v == i)]) >= 1:
                        if not meta[j][4] == None:
                            meta[j][4].remove()
                            meta[j][4] = None
                ...
                if f[0] >= 730:
                    meta[j][2][-1], = ax3.plot(x_ident[j][i], y_ident[j][i], marker='s', color=meta[j][1], markersize=8)
                    if meta[j][3] == None:
                    ...
                    meta[j][2][-1], = ax3.plot(x_ident[j][i], y_ident[j][i], marker='o', color=meta[j][1], markersize=8)
            ...
            if i % 100 == 0:
                ...
                current_fund = fund_v[current_indices][~np.isnan(fund_v[current_indices])]
...
def main():
    ...
    create_movie(ident_v, idents, idx_v, times, x_ident, y_ident, fund_v)
