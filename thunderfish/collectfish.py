...
import sys
import argparse
from thunderfish.version import __version__, __year__
    ...
    for file_name in files:
        ...
        if not table:
            ...
            if fish_type == 'wave':
                if harmonics is not None:
                    wave_spec = td.TableData(base_path + '-wavespectrum-0' + file_ext)
                    if data.nsecs > 0:
                ...
                if peaks0 is not None:
                    pulse_peaks = td.TableData(base_path + '-pulsepeaks-0' + file_ext)
                    if data.nsecs > 0:
        ...
        for r in range(data.rows()):
            ...
            if peaks0 is not None and fish_type == 'pulse':
                pulse_peaks = td.TableData(base_path + '-pulsepeaks-%d'%r + file_ext)
                for p in range(peaks0, peaks1+1):
                ...
                wave_spec = td.TableData(base_path + '-wavespectrum-%d'%r + file_ext)
                for h in range(harmonics+1):
...
def main():
    ...
    if not data_format:
        ...
        if ext in td.TableData.ext_formats:
            data_format = td.TableData.ext_formats[ext]
        else:
            data_format = 'dat'
    # write tables:
