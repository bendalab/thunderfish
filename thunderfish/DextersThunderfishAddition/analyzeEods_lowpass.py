...
def main():  ############################################################# Get arguments eodsfilepath, plot, (opt)save, (opt)new  
    ...
    if os.path.exists(filename[prefixlen:-4]+"_freqs2_lp.npy"):
        if os.path.getsize(filename[prefixlen:-4]+"_freqs2_lp.npy") > 0:
            print('exists already')
            quit()
        else:
            print('found a 0!')
    
   # if os.path.exists(filename[prefixlen:-4]+'_AmpFreq5_lp.pdf'):
   #       new = 0
...
#        np.save('classes/'+ filename[prefixlen:-4]+"_class%d_reduced"%i, rfish)
    ...
    window_freq = 1
    freqavgsecpath = filename[prefixlen:-4]+"_freqs2_lp.npy"
    if new == 1 or not os.path.exists(freqavgsecpath):
        print('new freq calcing')
        avg_freq = np.zeros([len(reducednpFishes),datalen//int((samplerate*window_freq))+1])
        avg_isi = np.zeros([len(reducednpFishes),datalen//int(samplerate*window_freq)+1])
        for i, fish in enumerate(reducednpFishes):
         if len(fish[0]) >= 5:
           ...
           # avg_freqs_temp, noiseindice = noisedelete_smoothing(avg_freqs_temp, 3, 2, 100000, 1000)
            ...
            avg_freq[i, int(fish[0][0])//int(samplerate*window_freq) : int(fish[0][0])//int(samplerate*window_freq)+sec] = np.array(avg_freqs_temp)
        #plt.show()
   ...
   ## #print(avg_freqs[0])
    ...
    window_avg = 1
    ampavgsecpath = filename[prefixlen:-4]+'_amps2_lp.npy'
        #freqtime = np.arange(0, len(data), samplerate)
    if new == 1 or not os.path.exists(ampavgsecpath):
        avg_amps_temp = []
        peak_ind = 0
        
        avg_amp = np.zeros([len(reducednpFishes),int(datalen)//int(samplerate*window_avg)+1])
        ...
        #avg_amps[:] = avg_amps_temp
            ...
            avg_amp[i, int(fish[0][0])//int(samplerate*window_avg) : int(fish[0][0])//int(samplerate*window_avg)+step] = avg_amps_temp
...
#        np.save(ampavgsecpath, avg_amp_fish)
    ...
    if new == 1 or plot == 1 or save == 1:
        # Plotting #######################################################################################################################
        ##################################################################################################################################

        plt.style.use('../PaperWholeFig.mplstyle')
        import matplotlib.gridspec as gridspec
        gs = gridspec.GridSpec(2, 2, height_ratios=(1, 1), width_ratios=(1, 0.02), wspace = 0.03)
      ...
      #  valuecount = 0
        ...
        for f, a in zip(avg_freq, avg_amp):
            ...
            for v in fc:
                print(v)
            col = [collist[int(v)] for v in fc[~np.isnan(fc)]]
       ...
       # ampax.legend(ls, labels, loc=0)
        ...
        ampbar.set_clim(-0.3,1)
        freqbar.set_ticks([0,30,60,90,120])
        ampbar.set_ticks(np.arange(0,1,0.249))
        ampbar.set_ticklabels([0,0.5,1,1.5,2])
        ...
        freqax.get_yaxis().set_label_coords(-0.05, 0.5)
        ampax.get_yaxis().set_label_coords(-0.05, 0.5)
        plt.gcf().subplots_adjust(bottom=0.15)
        ampbar.ax.get_yaxis().set_label_coords(4,0.5)
        ampbar.ax.set_ylabel('Amplitude', rotation=270)
        freqbar.ax.get_yaxis().set_label_coords(4,0.5)
        freqbar.ax.set_ylabel('Frequency', rotation=270)


      #  freqax.set_xlim(20600,21800)
      #  ampax.set_xlim(20600, 21800)
      #  freqax.set_ylim(80,125)
        ...
        if plot == 1:
            ...
            plt.show()
        if save == 1:
           plt.savefig(filename[prefixlen:-4]+'_AmpFreq7_lp.pdf')
           print('saved')
           plt.clf()
    else:
...
def bin_median(data, binlen): 
    return np.median(data[:(data.size // binlen) * binlen].reshape(-1, binlen),axis=1)
