...
def estimate_error(a_error, f_error, t_error, a_error_distribution, f_error_distribution,
    ...
    # f_e = f_weight * boltzmann(f_error, alpha=1, beta=0, x0=2.5, dx=.6)
...
def freq_tracking_v4(fundamentals, signatures, times, freq_tolerance, n_channels, return_tmp_idenities=False,
    ...
    def get_a_and_f_error_dist2(fund_v, idx_v, sign_v, start_idx, idx_comp_range, freq_lims, low_freq_th, high_freq_th,
                                freq_tolerance, a_error_distribution = np.array([]), f_error_distribution= np.array([])):
    ...
    for enu, i in enumerate(np.arange(len(fundamentals))):
        ...
        if enu % idx_comp_range == 0:
            ...
            if step_plot:
                ...
                tmp_handle = []
                for ident in np.unique(tmp_ident_v[~np.isnan(tmp_ident_v)]):
                    h, = ax.plot(times[idx_v[tmp_ident_v == ident]], fund_v[tmp_ident_v == ident], color='grey', lw=4, zorder = 0)
                    tmp_handle.append(h)
            ...
            layers, idx0s, idx1s = np.unravel_index(np.argsort(cp_error_cube[:idx_comp_range], axis=None), np.shape(cp_error_cube[:idx_comp_range]))
            ...
            if plotted:
                ...
                for ident in np.unique(tmp_ident_v[~np.isnan(tmp_ident_v)]):
                    ...
                    ax[7].fill_between([times[i], times[i + idx_comp_range * 3]], [.925, .925], [.95, .95], color='grey', alpha=0.7)
                    ax[7].fill_between([times[i + idx_comp_range], times[i + idx_comp_range * 2]], [.925, .925], [.95, .95], color='k')
                    ...
                    ax[7].plot([times[i + idx_comp_range * 2], times[i + idx_comp_range * 2]], [.55, 0.925], color='k', lw=.5)
                    ax[7].plot([times[i + idx_comp_range * 3], times[i + idx_comp_range * 3]], [.55, 0.925], color='k', lw=.5)
                    ax[7].plot([times[i], times[i]], [.1, 0.45], color='k', lw=.5)
                    ax[7].plot([times[i + idx_comp_range], times[i + idx_comp_range]], [.1, 0.45], color='k', lw=.5)
                    ax[7].plot([times[i + idx_comp_range * 2], times[i + idx_comp_range * 2]], [.1, 0.45], color='k', lw=.5)
                    ax[7].plot([times[i + idx_comp_range * 3], times[i + idx_comp_range * 3]], [.1, 0.45], color='k', lw=.5)
                    ...
                    ax[8].fill_between([times[i], times[i + idx_comp_range * 3]], [.925, .925], [.95, .95], color='grey', alpha=0.7)
                    ax[8].fill_between([times[i + idx_comp_range], times[i + idx_comp_range * 2]], [.925, .925], [.95, .95], color='k')
                    ...
                    ax[8].plot([times[i + idx_comp_range * 2], times[i + idx_comp_range * 2]], [.55, 0.925], color='k', lw=.5)
                    ax[8].plot([times[i + idx_comp_range * 3], times[i + idx_comp_range * 3]], [.55, 0.925], color='k', lw=.5)
                    ax[8].plot([times[i], times[i]], [.1, 0.45], color='k', lw=.5)
                    ax[8].plot([times[i + idx_comp_range], times[i + idx_comp_range]], [.1, 0.45], color='k', lw=.5)
                    ax[8].plot([times[i + idx_comp_range * 2], times[i + idx_comp_range * 2]], [.1, 0.45], color='k', lw=.5)
                    ax[8].plot([times[i + idx_comp_range * 3], times[i + idx_comp_range * 3]], [.1, 0.45], color='k', lw=.5)
                    ...
                    ax[3].plot(times[idx_v[(tmp_ident_v == ident)]], fund_v[(tmp_ident_v == ident)], lw=5, color=c, zorder=1, alpha=.4)
                    ...
                    ax[3].plot(times[idx_v[(tmp_ident_v == ident) & (idx_v > i + idx_comp_range) & (idx_v <= i + idx_comp_range*2)]],
                               fund_v[(tmp_ident_v == ident) & (idx_v > i + idx_comp_range) & (idx_v <= i + idx_comp_range*2)],
                               lw=5, color=c, zorder=2)

                    ax[4].plot(times[idx_v[(tmp_ident_v == ident)]], fund_v[(tmp_ident_v == ident)], lw=5, color=c, zorder=1, alpha=.4)
                    ...
                    ax[4].plot(times[idx_v[(tmp_ident_v == ident) & (idx_v > i + idx_comp_range) & (idx_v <= i + idx_comp_range*2)]],
                               fund_v[(tmp_ident_v == ident) & (idx_v > i + idx_comp_range) & (idx_v <= i + idx_comp_range*2)],
                               lw=5, color=c, zorder=2)

                    ax[5].plot(times[idx_v[(tmp_ident_v == ident)]], fund_v[(tmp_ident_v == ident)], lw=5, color=c, zorder=1, alpha=.4)
                    ...
                    ax[5].plot(times[idx_v[(tmp_ident_v == ident) & (idx_v > i + idx_comp_range) & (idx_v <= i + idx_comp_range*2)]],
                               fund_v[(tmp_ident_v == ident) & (idx_v > i + idx_comp_range) & (idx_v <= i + idx_comp_range*2)],
                               lw=5, color=c, zorder=2)
            ...
            for layer, idx0, idx1 in zip(layers[:i_non_nan], idx0s[:i_non_nan], idx1s[:i_non_nan]):
                ...
                idxs_i0 = p_idx_v[(p_ident_v == p_ident_v[p_i0_m[layer][idx0]]) & (p_idx_v > i + idx_comp_range) & (p_idx_v <= i + idx_comp_range*2)]
                idxs_i1 = p_idx_v[(p_tmp_ident_v == p_tmp_ident_v[p_i1_m[layer][idx1]]) & (np.isnan(p_ident_v)) & (p_idx_v > i + idx_comp_range) & (p_idx_v <= i + idx_comp_range*2)]

                if np.any(np.diff(sorted(np.concatenate((idxs_i0, idxs_i1)))) == 0):
                    continue

                if step_plot:
                    ax.plot([times[p_idx_v[p_i0_m[layer][idx0]]],  times[p_idx_v[p_i1_m[layer][idx1]]]], [p_fund_v[p_i0_m[layer][idx0]], p_fund_v[p_i1_m[layer][idx1]]], color='white', lw = 2, zorder = 10)
                # print(p_tmp_ident_v[p_i1_m[layer][idx1]])


                p_ident_v[(p_tmp_ident_v == p_tmp_ident_v[p_i1_m[layer][idx1]]) & (np.isnan(p_ident_v)) & (p_idx_v > i + idx_comp_range) & (p_idx_v <= i + idx_comp_range*2)] = p_ident_v[p_i0_m[layer][idx0]]

                if plotted:
                    # embed()
                    # quit()
                    if ioi_field[0] in np.arange(len(idx_v))[tmp_ident_v == tmp_ident_v[i1_m[layer][idx1]]]:
                        ax[5].plot(times[idx_v[i1_m[layer][idx1]]], fund_v[i1_m[layer][idx1]], marker='o', color='k', zorder = 3)
                        ax[5].plot(times[idx_v[i0_m[layer][idx0]]], fund_v[i0_m[layer][idx0]], marker='o', color='gold', zorder = 3)

                        help = np.arange(len(idx_v))[(ident_v == ident_v[ioi_field[1]]) & (idx_v <= i + idx_comp_range)][-1]
            ...
            for ident in np.unique(p_tmp_ident_v[~np.isnan(p_tmp_ident_v)]):
                if len(p_ident_v[p_tmp_ident_v == ident][~np.isnan(p_ident_v[p_tmp_ident_v == ident])]) == 0:
                    p_ident_v[(p_tmp_ident_v == ident) & (p_idx_v > i + idx_comp_range) & (p_idx_v <= i + idx_comp_range*2)] = next_identity
                    next_identity += 1
...
class Obs_tracker():
    def __init__(self, data, samplerate, start_time, end_time, channels, data_snippet_idxs, data_file, auto, fill_spec, **kwargs):
        ...
        # task lists
        self.t_tasks = ['track_snippet', 'track_snippet_show', 'track_snippet_live', 'plot_tmp_identities', 'check_tracking']
            ...
            if self.current_task == 'plot_tmp_identities':
                ...
                if self.active_vec_idx and hasattr(self.f_error_dist, '__len__') and hasattr(self.a_error_dist, '__len__'):
                    ...
                    for ident in np.array(possible_identities):
                        c = np.random.rand(3)
                        h, = self.main_ax.plot(self.times[tmp_idx_v[tmp_ident_v == ident]], tmp_fund_v[tmp_ident_v == ident], marker='o', color=c, linewidth=3, markersize=5)
    ...
    def buttonpress(self, event):
        ...
        if event.inaxes == self.main_ax:
            ...
            if self.current_task in ['connect_trace', 'delete_trace', 'zoom', 'cut_trace', 'group_delete', 'group_connect', 'group_reassign', 'track_snippet_show']:
            ...
            if self.current_task == 'track_snippet_show':
                if event.button == 1:
                    ...
                    if self.ioi_field[0] == None:
                        self.ioi_field[0] = self.active_vec_idx
                        self.ioi_field_handle[0] = self.field_ax0.imshow(self.sign_v[self.ioi_field[0]].reshape(8, 8).transpose()[::-1], cmap='jet', interpolation='gaussian')
                        self.ioi_field_marker[0], = self.main_ax.plot(self.times[self.idx_v[self.ioi_field[0]]], self.fund_v[self.ioi_field[0]], marker='o', color='green', markersize=5)

                    elif self.ioi_field[1] == None:
                        self.ioi_field[1] = self.active_vec_idx
                        self.ioi_field_handle[1] = self.field_ax1.imshow(self.sign_v[self.ioi_field[1]].reshape(8, 8).transpose()[::-1],cmap='jet', interpolation='gaussian')
                        self.ioi_field_marker[1], = self.main_ax.plot(self.times[self.idx_v[self.ioi_field[1]]],
                                                                      self.fund_v[self.ioi_field[1]], marker='o', color='orange', markersize=5)
                        ...
                        error = estimate_error(a_e, f_e, np.abs(self.times[self.idx_v[self.ioi_field[1]]] - self.times[self.idx_v[self.ioi_field[0]]]), self.a_error_dist, self.f_error_dist)
                        self.error_text[0] = self.main_fig.text(.55, .1,'a_error: %.2f; f_error: %.2f; t_error: %.2f (%.1f s) \ntotal_error: %.2f' % (
                            error[0], error[1], error[2], np.abs(self.times[self.idx_v[self.ioi_field[1]]] - self.times[self.idx_v[self.ioi_field[0]]]), error[0] * 2./3 + error[1] * 1./3), color='orange')

                        self.ioi_a_error_line[0][0],  = self.a_error_ax.plot([a_e, a_e], [0, rel_a_e], color='orange')
                        self.ioi_a_error_line[0][1],  = self.a_error_ax.plot([0, a_e], [rel_a_e, rel_a_e], color='orange')

                        self.ioi_f_error_line[0][0], = self.f_error_ax.plot([f_e, f_e], [0, rel_f_e], color='orange')
                        self.ioi_f_error_line[0][1], = self.f_error_ax.plot([0, f_e], [rel_f_e, rel_f_e], color='orange')

                    else:
                        self.ioi_field[2] = self.active_vec_idx
                        self.ioi_field_handle[2] = self.field_ax2.imshow(self.sign_v[self.ioi_field[2]].reshape(8, 8).transpose()[::-1],cmap='jet', interpolation='gaussian')
                        self.ioi_field_marker[2], = self.main_ax.plot(self.times[self.idx_v[self.ioi_field[2]]],
                                                                      self.fund_v[self.ioi_field[2]], marker='o', color='red',
                                                                      markersize=5)
                        ...
                        error = estimate_error(a_e, f_e, np.abs(self.times[self.idx_v[self.ioi_field[2]]] - self.times[self.idx_v[self.ioi_field[0]]]), self.a_error_dist, self.f_error_dist)
                        self.error_text[1] = self.main_fig.text(.55, .55, 'a_error: %.2f; f_error: %.2f; t_error: %.2f (%.1f s) \ntotal_error: %.2f' % (
                            error[0], error[1], error[2], np.abs(self.times[self.idx_v[self.ioi_field[2]]] - self.times[self.idx_v[self.ioi_field[0]]]), error[0] * 2./3 + error[1] * 1./3), color='red')
    ...
    def method_figure(self):
        ...
        if True:
            ...
            spec = np.memmap(os.path.join(os.path.split(self.data_file)[0], 'fill_spec.npy'), dtype='float', mode='r', shape=tuple(shape), order='F')
            ...
            #####
            ax[9].plot(self.times[self.idx_v[self.ioi_field[0]]], self.fund_v[self.ioi_field[0]], marker='o', color='k', markersize=5)
            ax[10].plot(self.times[self.idx_v[self.ioi_field[0]]], self.fund_v[self.ioi_field[0]], marker='o', color='k', markersize=5)
            ax[9].plot(self.times[self.idx_v[self.ioi_field[1]]], self.fund_v[self.ioi_field[1]], marker='o', color='forestgreen', markersize=5)
            ax[10].plot(self.times[self.idx_v[self.ioi_field[1]]], self.fund_v[self.ioi_field[1]], marker='o', color='forestgreen', markersize=5)
            ax[9].plot(self.times[self.idx_v[self.ioi_field[2]]], self.fund_v[self.ioi_field[2]], marker='o', color='gold', markersize=5)
            ax[10].plot(self.times[self.idx_v[self.ioi_field[2]]], self.fund_v[self.ioi_field[2]], marker='o', color='gold', markersize=5)
            ...
            ax[10].plot([lt1, lt1], [self.fund_v[self.ioi_field[0]], self.fund_v[self.ioi_field[1]]], lw=4, color='forestgreen', label='$\Delta f_0$')
            ax[10].plot([lt2, lt2], [self.fund_v[self.ioi_field[0]], self.fund_v[self.ioi_field[2]]], lw=4, color='gold', label='$\Delta f_1$')
        ...
        if True:
            ...
            ax0.plot((np.arange(len(s_idsf))+1) / dps, aucs_f, marker = '.', color = colors[6], zorder=0, label='frequency')
        ...
        if True:
            ...
            cax = ax1.imshow(self.sign_v[self.ioi_field[0]].reshape(8, 8).transpose()[::-1], cmap='jet', interpolation='gaussian', vmin=0, vmax=1)
            ax2.imshow(self.sign_v[self.ioi_field[1]].reshape(8, 8).transpose()[::-1], cmap='jet', interpolation='gaussian', vmin=0, vmax=1)
            ax22.imshow(dsig0.reshape(8, 8).transpose()[::-1], cmap='jet', interpolation='gaussian', vmin=0, vmax=1)
            ax3.imshow(self.sign_v[self.ioi_field[2]].reshape(8, 8).transpose()[::-1], cmap='jet', interpolation='gaussian', vmin=0, vmax=1)
        ...
        if True:
            ...
            a_e = np.sqrt(np.sum((self.sign_v[self.ioi_field[0]] - self.sign_v[self.ioi_field[1]]) ** 2))
            rel_a_e = len(self.a_error_dist[self.a_error_dist <= a_e]) / len(self.a_error_dist)
            ax[0].plot([a_e, a_e], [0, rel_a_e], color='gold', alpha=.7, lw=3, label='$\Delta field_0$ = %.2f' % rel_a_e)
            ax[0].plot([0, a_e], [rel_a_e, rel_a_e], color='gold', alpha=.7, lw=3)

            a_e = np.sqrt(np.sum((self.sign_v[self.ioi_field[0]] - self.sign_v[self.ioi_field[2]]) ** 2))
            rel_a_e = len(self.a_error_dist[self.a_error_dist <= a_e]) / len(self.a_error_dist)
            ax[0].plot([a_e, a_e], [0, rel_a_e], color='forestgreen', alpha=.7, lw=3, label='$\Delta field_1$ = %.2f' % rel_a_e)
        ...
        if True:
            ...
            f_e = np.abs(self.fund_v[self.ioi_field[0]] - self.fund_v[self.ioi_field[1]])
            rel_f_e = boltzmann(f_e, alpha=1, beta=0, x0=.25, dx=.15)
            ax[1].plot([f_e, f_e], [0, rel_f_e], color='gold', alpha=.7, lw=3, label='$\Delta frequency_0$ = %.2f' % rel_f_e)
            ax[1].plot([0, f_e], [rel_f_e, rel_f_e], color='gold', alpha=.7, lw=3)

            f_e = np.abs(self.fund_v[self.ioi_field[0]] - self.fund_v[self.ioi_field[2]])
            rel_f_e = boltzmann(f_e, alpha=1, beta=0, x0=.25, dx=.15)
            ax[1].plot([f_e, f_e], [0, rel_f_e], color='forestgreen', alpha=.7, lw=3, label='$\Delta frequency_1$ = %.2f' % rel_f_e)
    ...
    def plot_spectrum(self, part_spec = False, part_spec_from_file=False):
        if part_spec_from_file:
            ...
            if os.path.exists(os.path.join(os.path.split(self.data_file)[0], 'fill_freqs.npy')):
                ...
                self.part_t_lims = np.arange(len(self.fill_times))[(self.fill_times >= limitations[0] + self.spec_shift) & (self.fill_times <= limitations[1] + self.spec_shift)]
    ...
    def track_snippet(self):
        ...
        if not hasattr(self.fund_v, '__len__'):
            ...
            mask = np.arange(len(self.idx_v))[(self.times[self.idx_v] >= snippet_start) & (self.times[self.idx_v] <= snippet_end)]
            self.fundamentals = []
            self.signatures = []
            ...
            # time_limits = self.main_ax.get_xlim()
            # self.fundamentals = np.array(self.fundamentals)[(self.times >= time_limits[0]) & (self.times <= time_limits[1])]
            # self.signatures = np.array(self.signatures)[(self.times >= time_limits[0]) & (self.times <= time_limits[1])]
            # self.times = self.times[(self.times >= time_limits[0]) & (self.times <= time_limits[1])]
    ...
    def plot_error(self):
        ...
        self.f_error_ax.plot(np.arange(0, 5, 0.02), boltzmann(np.arange(0, 5, 0.02), alpha=1, beta=0, x0=.25, dx=.15), color='cornflowerblue', linewidth=2)
    ...
    def plot_traces(self, clear_traces = False, post_connect= False, post_cut=False, post_delete=False, post_group_delete=False, post_group_connection=False, post_refill=False, post_group_reassign=False):
        """
    ...
    def plot_ps(self):
        ...
        groups, _, _, self.all_peakf, self.good_peakf, self.kwargs['low_threshold'], self.kwargs['high_threshold'], self.psd_baseline = harmonic_groups(all_c_freqs, self.power, verbose=self.verbose, **self.kwargs)
        self.verbose = 0
...
def fish_tracker(data_file, start_time=0.0, end_time=-1.0, grid=False, auto = False, fill_spec = False, transect_data = False, data_snippet_secs=15., verbose=0, **kwargs):
    """
...
def main():
    ...
    fish_tracker(datafile, args.start_time * 60.0, args.end_time * 60.0, args.grid, args.auto, args.fill_spec, args.transect_data, **t_kwargs)
