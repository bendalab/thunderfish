...
def plot_data(data, rate, peak_idx, trough_idx, idx0, idx1,
              win_times, peak_rate, mean_ampl, cv_ampl, valid_wins, ax, fs=10) :
    # raw data:
    ...
    ax[0].plot(time[peak_idx], data[peak_idx], 'o', mfc='crimson', mec='k', mew=2., ms=12)
    ax[0].plot(time[trough_idx], data[trough_idx], 'o', mfc='lime', mec='k', mew=2., ms=12)
    ...
    # peak rate:
    ax[1].plot(win_times, peak_rate, 'o', ms=10, color='grey', mew=2., mec='black', alpha=0.6)
    ax[1].plot(win_times[valid_wins], peak_rate[valid_wins], 'o', ms=10, color='red', mew=2., mec='black', alpha=0.6)
    ax[1].set_ylabel('EOD rate [Hz]', fontsize=fs)
...
def plot_window(lower_rate_th, upper_rate_th, ampl_th, cv_th, ax, fs=10) :
    ax[1].axhline(lower_rate_th, color='k')
    ax[1].axhline(upper_rate_th, color='k')
    ax[2].axhline(ampl_th, color='k')
    ax[3].axhline(cv_th, color='k')
