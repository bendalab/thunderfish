...
chirps_frequency() for chirps.

The returned frequency can be directly passed on to generate_wavefish()
for generating a frequency modulated EOD waveform.
"""
...
def chirps_frequency(eodf=100.0, samplerate=44100., duration=1.,
    ...
    chirp_freq: float
        Frequency of the chirps in Hertz.
    chirp_size: float
    ...
    chirp_times = np.arange(0.5*chirp_period, duration-0.5*chirp_period, chirp_period)
...
if __name__ == '__main__':
    ...
    # chirps:
    freq = chirps_frequency(600.0, samplerate, duration=duration, chirp_kurtosis=1.0)
    data = generate_alepto(freq)

    # plot:
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(14, 8))
    ax.set_title('Chirps')
    nfft = 256
    ax.specgram(data, Fs=samplerate, NFFT=nfft, noverlap=nfft/16)
    time = np.arange(len(freq))/samplerate
    ax.plot(time[:-nfft/2], 3*freq[nfft/2:], '-k', lw=2)
